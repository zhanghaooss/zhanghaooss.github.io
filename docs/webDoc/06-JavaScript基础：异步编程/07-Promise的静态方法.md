...
-   `Promise.any()`:任何一个promise状态敲定即可，无论成功与否 通常用不到
...
function get(url) {
    ...
    var a = 'http://api.xin88.top/bilibili/news.json'
    var b = 'http://api.xin88.top/bilibili/timeline.json'
    var c = 'http://api.xin88.top/bilibili/recommend.json'
    var d = 'http://api.xin88.top/bilibili/recommends.json'
    // 高效写法
    Promise.all([
    ...
    Promise.allSettled([
      ...
      console.log(err);
    })//增加了状态值 无论每个是否成功 所有执行一遍后执行.then
