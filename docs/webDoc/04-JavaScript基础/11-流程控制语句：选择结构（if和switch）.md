...
上面的写法 2，是比较推荐的写法：直接通过 **return 的方式，让 function 里的代码不再继续往下走**，这就达到目的了。对了，<u>**因为要用到 return ，所以整段代码是封装到一个 function 里的**</u>。
