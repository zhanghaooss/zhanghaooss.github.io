...
备注：<u>`indexOf()` 是从前向后查找字符串的位置。同理，`lastIndexOf()`是从后向前寻找。</u>

**解释**：<u>可以检索一个字符串中是否含有指定内容</u>。如果字符串中含有该内容，则会返回其**第一次出现**的索引；如果没有找到指定的内容，则返回 -1。
...
<u>这个方法还可以指定第二个参数，用来指定查找的**起始位置**。语法如下：</u>
...
<u>在实际应用中，通过这个方法，我们可以判断用户按下了哪个按键。</u>
...
备注：`split()`这个方法在实际开发中用得非常多。一般来说，<u>从接口拿到的 json 数据中，经常会收到类似于`"q, i, a, n"`这样的字符串，前端需要将这个字符串拆分成`['q', 'i', 'a', 'n']`数组</u>，这个时候`split()`方法就派上用场了。
