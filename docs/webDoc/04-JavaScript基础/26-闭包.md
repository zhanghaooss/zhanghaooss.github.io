...
闭包（Closure）是指一个函数（函数A）内部定义的函数（函数B），函数B可以引用函数A中的局部变量，即使函数A已经执行完毕并返回，函数B仍然可以使用这些局部变量。这种特性使得闭包可以用来保存函数的状态，实现函数的记忆化等高级应用。
...
一种称呼，代表函数作用域的一种状态：被其他函数存储在scopes中 无法释放
...
比如有这样一个需求：定义一个私有变量a，要求a只能被进行指定操作（加减），不能进行其他操作（乘除）。在  Java、C++ 等语言中，有==私有属性==的概念，但在JS中只能通过闭包模拟。
...
> 垃圾回收 引用类型数据不再被任何的地址所引用就会自动销毁，对象啊数组啊函数都赋值null就会销毁
