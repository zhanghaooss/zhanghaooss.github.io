...
> 获取到的dom元素数组是类数组，但是它的原型中存在foreach方法
...
事件触发时会接收到一个实参，可以用arguments查看，写个形参接收就可以
